...
export const VoiceRoom: React.FC<VoiceRoomProps> = ({ roomId, onClose }) => {
  ...
  const generateToken = async () => {
    ...
    const response = await fetch(`${apiUrl}/app/rooms/${roomId}/voice/token`, {
      method: 'POST',
      headers: {
  ...
  const notifyJoinRoom = async () => {
    try {
      const token = localStorage.getItem('accessToken') || localStorage.getItem('authToken');
      const apiUrl = import.meta.env.VITE_BASE_URL || import.meta.env.VITE_API_URL || 'http://localhost:4005/api/v1';
      await fetch(`${apiUrl}/app/rooms/${roomId}/voice/join`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${token}`,
  ...
  const notifyLeaveRoom = async () => {
    try {
      const token = localStorage.getItem('accessToken') || localStorage.getItem('authToken');
      const apiUrl = import.meta.env.VITE_BASE_URL || import.meta.env.VITE_API_URL || 'http://localhost:4005/api/v1';
      await fetch(`${apiUrl}/app/rooms/${roomId}/voice/leave`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${token}`,
    ...
    <div className="voice-room">
      <div className="voice-room-header">
        <div className="status-indicator">
          ...
          <span className="text">Appel vocal en cours</span>
        </div>
      ...
      <div className="voice-controls">
        ...
        <button className="control-btn danger" onClick={handleLeave}>
          ...
          <span className="label">Raccrocher</span>
        </button>
      </div>
    </div>
  );
